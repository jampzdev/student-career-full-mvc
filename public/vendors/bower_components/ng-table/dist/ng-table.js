    ...
    this.compileDirectiveTemplates = function () {
        if (!$element.hasClass('ng-table')) {
            $scope.templates = {
                ...
                pagination: ($attrs.templatePagination ? $attrs.templatePagination : 'ng-table/pagerjmpz.html')
            };
            ...
            if($element.parent().hasClass("table-alt-pagination")){
                $element.closest(".table-alt-pagination").after(paginationTemplate);
            }
            else{
                console.log($element.parent().innerWidth());
                console.log($element.innerWidth());
                console.log($element.children().first().children().children().length);
                $element.parent().after(paginationTemplate);
            }

            // if($element.parent().innerWidth() > screen.width){
            if($element.children().first().children().children().length > 20){
...
angular.module('ngTable').run(['$templateCache', function ($templateCache) {
	...
	$templateCache.put('ng-table/pagerjmpz.html', '<div class="ng-cloak ng-table-pager"> <div class="ng-table-counts btn-group pull-right"> <button ng-repeat="count in params.settings().counts" type="button" ng-class="{\'active\':params.count()==count}" ng-click="params.count(count)" class="btn btn-default"> <span ng-bind="count"></span> </button> </div> <ul class="pagination ng-table-pagination"> <li ng-class="{\'enabled\': !page.active && !page.current, \'active\': page.current}" ng-repeat="page in pages" ng-switch="page.type"> <a ng-switch-when="prev" ng-click="params.page(page.number)" href="">&laquo;</a> </span></a> <a ng-switch-when="next" ng-click="params.page(page.number)" href="">&raquo;</a> </li> </ul> </div> ');
